# if i pass this code a string 'execute', it doesn't give an appropriate output
# output should be RESERVEDWORD: 'exec', NOISEWORD: 'ute'
...
TT_INT                  = 'INT:                           '
TT_FLOAT                = 'FLOAT:                         '
TT_LPAREN               = 'DEL_LEFTPAREN:                 ' + '('
TT_RPAREN               = 'DEL_RIGHTPAREN:                ' + ')'
TT_SCOLON               = 'DEL_SEMICOLON:                 ' + ';'
TT_COLON                = 'COLON:                         ' + ':'
TT_OCBRACE              = 'DEL_OPENBRACE:                 ' + '{'
TT_CCBRACE              = 'DEL_CLOSEBRACE:                ' + '}'
...
TT_ADD                  = 'ARITHMETIC_ADD:                ' + '+'
TT_SUB                  = 'ARITHMETIC_SUBTRACT:           ' + '-'
TT_MUL                  = 'ARITHMETIC_MULTIPLY:           ' + '*'
TT_DIV                  = 'ARITHMETIC_DIVIDE:             ' + '/'
...
class Lexer:
    ...
    def __init__(self, text):
        self.text = text
        self.pos = Position(-1, 0, -1, None, text)
    ...
    def make_tokens(self):
        ...
        while self.current_char is not None:
            ...
            # Check for single-line comment
                ...
                tokens.append(Token(TT_SINGLECOMMENT, pos_start=self.pos))
                tokens.append(Token(TT_COMMENT, comment_text, pos_start=self.pos))

            # Check for multi-line comment
            elif self.current_char == '/' and self.peek() == '*':
                comment_text = self.make_multi_line_comment()
                tokens.append(Token(TT_STARTCOMMENT, pos_start=self.pos))
                tokens.append(Token(TT_COMMENT, comment_text, pos_start=self.pos))
                tokens.append(Token(TT_ENDCOMMENT, pos_start=self.pos))
                ...
                if isinstance(result, Token):
                    tokens.append(result, pos_start=self.pos)
                else:
                ...
                if self.current_char == '{':
                    tokens.append(Token(TT_OCBRACE, pos_start=self.pos))
                    self.advance()
                elif self.current_char == '}':
                    tokens.append(Token(TT_CCBRACE, pos_start=self.pos))
                    self.advance()

            elif self.current_char in (ASSIGNOPRTR or BOOLOPRTR or SPECIALCHAR):
                if self.current_char == '=':
                    if self.peek() == '=':
                        tokens.append(Token(TT_EQUIVAL, pos_start=self.pos))
                        self.advance()
                        self.advance()
                    else:
                        tokens.append(Token(TT_ASSIGNMENT_EQUAL, pos_start=self.pos))
                        self.advance()
                elif self.current_char == ':':
                    tokens.append(Token(TT_ASSIGNMENT_COLON, pos_start=self.pos))
                    self.advance()
                ...
                self.advance()
                statement_tokens = []
                tokens.extend(statement_tokens, pos_start=self.pos)
                ...
                if self.peek() == '&':
                    ...
                    tokens.append(Token(TT_SPECIALCHAR, '&', pos_start=self.pos))
                    self.advance()
                ...
                if self.current_char == '!':
                    if self.peek() == '=':
                        tokens.append(Token(TT_NOTEQUAL, pos_start=self.pos))
                        self.advance()
                        self.advance()
                    else:
                    ...
                    if self.peek() == '=':
                        tokens.append(Token(TT_LEQUAL, pos_start=self.pos))
                        self.advance()
                        self.advance()
                    else:
                    ...
                    if self.peek() == '=':
                        tokens.append(Token(TT_GEQUAL, pos_start=self.pos))
                        self.advance()
                        self.advance()
                    else:
...
class BinOpNode:
    ...
    def __repr__(self):
        return f'({repr(self.left_node)}, {repr(self.op_tok)}, {repr(self.right_node)})'
...
def run(filename):
    try:
        ...
        if file_extension != '.supp':  # Replace '.your_specific_extension' with the desired file extension
